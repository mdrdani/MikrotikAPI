...
<body>
	...
	<div class="main-content">
			...
			<?php
				...
				$responses1 = $client->sendSync(new RouterOS\request('/ip/address/print'));
				?>
			...
			<table class="table table-bordered table-responsive-lg">
				...
				<?php foreach($responses1 as $rs1) :?>
					...
					<tr>
					<th scope="row"><?= $i; ?></th>
					<td><?php echo $rs1->getProperty('address'); ?></td>
					<td><?php echo $rs1->getProperty('network'); ?></td>
					<td><?php echo $rs1->getProperty('interface'); ?></td>
					<td><?php echo $rs1->getProperty('dynamic'); ?></td>

					<?php if($rs1->getProperty('dynamic') === 'false'){?>
					<td>
					<a class="btn btn-danger" href="delipaddress.php?id=<?php echo $rs1->getProperty('.id'); ?>" role="button">Hapus</a>
					</td>
					<?php } ?>
					</tr>
					<?php $i++; ?>
