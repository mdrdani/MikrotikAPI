...
<body>
	...
	<div class="main-content">
				<h1>Daftar IP DNS (Domain Name System)</h1>
				<br>
		<?php
			$client = new RouterOS\Client($_SESSION["ipaddress"], $_SESSION["username"], $_SESSION["password"]);
			$responses1 = $client->sendSync(new RouterOS\request('/ip/dns/print'));
			?>
			<table class="table table-bordered table-responsive-lg">
				<thead class="thead-dark">
					<tr>
					<th scope="col">Static Servers</th>
					<th scope="col">Dynamic Servers</th>
					<th scope="col">Allow-Remote</th>
				...
				<?php foreach($responses1 as $rs1) :?>
					<?php if($rs1->getType() == RouterOS\Response::TYPE_DATA){?>
							...
							<?php if($rs1->getProperty('servers')){ ?>
								<tr>
									<td><?php echo $rs1->getProperty('servers'); ?></td>
									<td></td>
									<td><?php echo $rs1->getProperty('allow-remote-requests'); ?></td>
									<td>
									<a class="btn btn-danger" href="delipdns.php?servers=<?php echo $rs1->getProperty('servers')?>" role="button">Hapus</a>
									</td>
								</tr>
								
								<tr>
									<td></td>
									<td><?php echo $rs1->getProperty('dynamic-servers'); ?></td>
									<td><?php echo $rs1->getProperty('allow-remote-requests'); ?></td>
								</tr>
							<?php }else{ ?>
								<tr>
